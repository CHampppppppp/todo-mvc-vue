<template>
  <section class="todoapp">
    <header class="header">
      <h1>todos</h1>
      <input class="new-todo" placeholder="What needs to be done?" autofocus v-model="newTodoTitle" @keydown.enter="addTodo">
    </header>
    ...
    <section class="main">
      ...
      <ul class="todo-list">
		...
		:class="{ completed : todo.completed }">
          <div class="view"> 
			<!-- 动态绑定for和id属性 -->
            <input class="toggle" type="checkbox" :id="todo.id" checked v-model="todo.completed">
            <label :for="todo.id">{{ todo.title }}</label>
            <button class="destroy" @click="destory(todo.id)"></button>
          </div>
    ...
    <footer class="footer" v-if="todos.length > 0">
      ...
      <button class="clear-completed" @click="clearCompleted" v-if="completedCount">Clear completed</button>
    </footer>
  </section>
</template>
